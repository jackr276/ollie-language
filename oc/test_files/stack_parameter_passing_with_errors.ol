...
pub fn! involved_function_float(x:f32, y:f32, z:f32, aa:f32, bb:f32, cc:f32, dd:f32, ee:f32, gg:f32) -> f32 raises(generic_error_t) {
	if(x - y == 0){
		raise generic_error_t;
	}

	ret x + y + z + aa + bb + cc + dd + ee + gg;
}
...
pub fn main() -> i32 {
	let x:f32 = @involved_function_float(32.3, .3, .4, .5, .6, .7, .8, .9, 1.0) handle(
																				generic_error_t => -1.0,
																				error => -1.0
																				);
