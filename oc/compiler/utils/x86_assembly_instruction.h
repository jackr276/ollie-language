...
typedef enum{
	...
	// 4(%rax)
	ADDRESSING_MODE_OFFSET_ONLY, 
 	// (%rax, %rcx)
	ADDRESSING_MODE_REGISTERS_ONLY,
	// 4(%rax, %rcx)
	ADDRESSING_MODE_REGISTERS_AND_OFFSET,
	// (%rax, %rcx, 8)
	ADDRESSING_MODE_REGISTERS_AND_SCALE,
	// 4(%rax, %rcx, 8)
	ADDRESSING_MODE_REGISTERS_OFFSET_AND_SCALE,
	// (, %rcx, 8)
	ADDRESSING_MODE_INDEX_AND_SCALE, 
	// 4(, %rcx, 8)
	ADDRESSING_MODE_INDEX_OFFSET_AND_SCALE,
	//RIP relative addresing like: <val>(%rip)
	ADDRESSING_MODE_RIP_RELATIVE,
	//RIP relative addresing like: <offset> + <val>(%rip)
	ADDRESSING_MODE_RIP_RELATIVE_WITH_OFFSET 
...
typedef enum{
	...
	//Conditional moves - these only exist for word, long and quad word
	CMOVEW,
	CMOVEL,
	CMOVEQ,
	CMOVNEW,
