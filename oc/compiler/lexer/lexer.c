...
static Lexer_item identifier_or_keyword(char* lexeme, u_int16_t line_number){
	...
	const Token tok_arr[] = {IF, THEN, ELSE, DO, WHILE, FOR, FN, RET, JUMP, LINK, REPLACE, 
						...
						CONSTRUCT, AS, ALIAS, SIZEOF, DEFER, REPLACE, MUT, DEPENDENCY, ASM};
	...
	char* keyword_arr[] = {"if", "then", "else", "do", "while", "for", "fn", "ret", "jump",
								 ...
								 "i16", "u32", "i32", "u64", "i64", "f32", "f64", 
								  ...
								  "break", "continue", "construct", "as", "alias", "sizeof", "defer", "#replace", "mut", "#dependency", "_asm_"};

	//Let's see if we have a keyword here
	for(u_int8_t i = 0; i < 49; i++){
