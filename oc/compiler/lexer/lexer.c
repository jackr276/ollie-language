...
/**
 * Generate only the first 2 ollie tokens and store them inside of a pre-allocated
 * buffer
 */
static u_int8_t tokenize_first_2(FILE* fl, lexitem_t tokens[2], u_int8_t silent_mode){
	...
	//Maintain the current token index
	...
	while((ch = GET_NEXT_CHAR(fl)) != EOF){
		/**
		 * We only have room for 2 - so if this is 2 then we're
		 * done
		 */
					...
					case '/':
						...
						switch(ch2){
							...
							case '=':
								...
								tokens[current_token_index++] = lex_item;
								break;
