...
static basic_block_t* visit_if_statement(generic_ast_node_t* if_stmt_node, basic_block_t* function_block_end){
	...
	//All blocks flow through the end block(with the exception of returns)
	basic_block_t* end_block = basic_block_alloc();
	...
	//Assert that this is true -- for development uses
	if(cursor->CLASS != AST_NODE_CLASS_COMPOUND_STMT){
		print_parse_message(PARSE_ERROR, "Expected compund statement in if statement", cursor->line_number);
		exit(1);
	}
	...
	if(cursor == NULL){
		//We'll need the fail case to point to the "end block" if it returns here
		if(rets_through_1 == 1){
...
static basic_block_t* visit_function_declaration(generic_ast_node_t* func_def_node){
	...
	while(compound_block_end->direct_successor != NULL && compound_block_end->direct_successor->is_exit_block == 0){
		//Keep drilling down
		compound_block_end = compound_block_end->direct_successor;
	}
